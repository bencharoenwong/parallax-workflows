...
- **Derived Parallax token meter** — `evals/graders/token_model.py` drops the separate unverified-token accumulator and `RunRecord.parallax_tokens_unverified`, since every endpoint the table *prices* now carries a measured cost. This is not full coverage of the live MCP surface: `etf_search`, `etf_holdings`, `check_job_status`, and `submit_feedback` remain unpriced, so a workflow touching them (e.g. `parallax-pair-finder`) is classified `STATUS_DEGRADED` via `unknown_endpoints`. That is the designed stale-table signal, not a regression — pricing them by inference would mean publishing numbers nobody measured. A tool name absent from the table stays a hard failure, not a free call.
