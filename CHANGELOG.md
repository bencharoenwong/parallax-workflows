...
- **Derived Parallax token meter** — `evals/graders/token_model.py` drops the separate unverified-token accumulator and `RunRecord.parallax_tokens_unverified`, since every endpoint the table *prices* now carries a measured cost. This is not full coverage of the live MCP surface: `etf_search`, `etf_holdings`, `check_job_status`, and `submit_feedback` remain deliberately unpriced and are listed in the grader's `KNOWN_UNPRICED` set, so a workflow touching them (e.g. `parallax-pair-finder`) stays `STATUS_OK` and reports the calls separately as `unpriced_endpoints`, stating what the derived total omits — pricing them by inference would mean publishing numbers nobody measured. An endpoint under a recognised Parallax namespace that is genuinely absent from both the price table and `KNOWN_UNPRICED` still classifies the run `STATUS_DEGRADED` via `unknown_endpoints` — the designed stale-table signal, a hard failure rather than a free call.
- **Endpoint attribution by namespace, not tool name** — the meter bills a call only when its MCP namespace is recognisably Parallax: it contains `parallax`, or it is named in the new `PARALLAX_MCP_ALIASES` environment variable (comma-separated namespace segments, for connectors mounted under an off-brand alias — what white-label deployments need). A name carrying no MCP namespace is harness-local by construction and ignored, so the hand-maintained harness-tool blocklist is deleted rather than extended and can no longer drift. A *known* endpoint name arriving under an unrecognised namespace is reported as ambiguous: billed to nobody, listed in `RunRecord.ambiguous_endpoints`, and the run degrades — a tool name cannot prove which server served it, and the same name can be exposed by more than one connector, so both billing it and dropping it fail silently. Ambiguity is resolvable in either direction: `PARALLAX_MCP_ALIASES` declares the namespace to be the connector and starts billing its calls, while `PARALLAX_MCP_FOREIGN_NAMESPACES` declares it a different server whose tool name merely collides, so its calls are ignored. Offering only the first would force an operator to mislabel a foreign server as Parallax just to clear the degrade.
- **Single infra-failure predicate** — `grade_corpus` and `runrecord` now share `evals/graders/infra_failure.py` instead of keeping two drifting copies, so a run cannot be excluded from one report and averaged into another. Connector state is read from the stream's `system/init` event rather than by scanning the transcript for the connector's name (which is present in every captured stream, including one where the connector failed to load).
- **Grader price-table drift corrected** — `list_macro_countries` and `get_telemetry` were carried at 0 in the grader while `token-costs.md` (the module's stated source of truth) lists both under "1 token each". The grader now matches the published table, and `test_token_model.py` pins the two against each other so they cannot silently diverge again.
...
### Added
- **`parallax-conventions.md §11 Verdict Sensitivity`** — new convention: any skill that renders a threshold-driven verdict, screen, badge, or flag also states, in third person and grounded purely in arithmetic, which 1-2 inputs sit nearest their published cutoff and what value would flip the classification. It is a restatement of deterministic threshold logic already computed during the workflow — no new claim, no forecast, and no advice language (§11.4 forbidden-language list applies). §11.2 gates it to purely-arithmetic verdicts only; discrete or compound criteria are explicitly excluded. Wired into the AI-profile family output template, the screen/single-stock outputs, and the health-flag family. `parallax-score-explainer`'s existing "What Would Change It" line is the grandfathered precedent and is intentionally left as-is.
- **`parallax-conventions.md §12 Information Framing (Advice Boundary)`** — codifies that rendered classifications and candidate actions are observational statements about threshold logic, never execution instructions. Defines the permitted capitalized action-type labels (each must cite the matrix/flag that produced it), the required one-line informational preface, descriptive rationale verbs, and an `action_labels=plain` neutral-status render mode for retail-suitable deployments.
- **`parallax-conventions.md §13 Audience Render Mode`** — a presentation-only `client_safe` vs `internal_analyst` (default) render mode, resolved from a per-invocation flag, then white-label config, else default. `client_safe` glosses factor names in plain language, omits analyst apparatus (verdict-sensitivity lines, cutoff arithmetic), relocates methodology/ops apparatus to a trailing internal appendix (compliance-contract content is relocated, never deleted), and stamps `Audience mode: client-safe` in the footer. §13.2 enumerates the surfaces that are non-suppressible in any mode: the §9.2 AI-interaction banner, the §9.1/bespoke and AI-profile disclaimers, data-integrity mismatch warnings, the House View Preamble, and the footer. The mode is presentation-only and never alters the underlying verdict. Render gate extended to `should-i-buy` and `score-explainer` alongside the portfolio family.
- **`AI-profiles/methodology-faq.md`** — compliance-facing FAQ on the named-investor framing used by the `parallax-ai-*` family (what a profile is, why a real investor is named, misrepresentation guards, disclaimer and citation enforcement). Sourced only from repo-public rationale.
- **Translation validators** — `translate-chinese-finance` and `translate-thai-finance` each gain a deterministic pass/fail `validate-translation.py` check with a `--waive` escape hatch and a test suite.
- **Fund/OEIC resolution fallback (`parallax-conventions.md §1`)** — when symbol resolution fails after the standard attempts and the identifier looks like a fund, OEIC, or investment trust, skills now state that coverage is listed equities and ETFs only and continue processing remaining symbols, instead of surfacing a raw resolution error. Skills that JIT-load §1 inherit the fallback with no per-skill edit.
- **Localization boundary (`parallax-white-label-stock-report/references/field-map.md`)** — documents which branded-report field paths are prose-vs-data for a future localized-report project, and records that a localized branded report (renderer i18n, CJK/Thai font stacks, translated compliance-approved disclosures) is out of scope until specified as its own project.
- **`docs/security/audit-2026-07-15.md`** — pre-launch security audit covering this branch's diff (`CRITICAL_FAILS=0`); `audit-latest.md` symlink repointed. Consumed by the pre-push security gate.

### Changed
- **Report footer section renamed `Provenance` → `About This Report`** across consumer skills, the §9.2 AI-interaction banner text, eval graders, eval configs, and fixtures. The house-view provenance machinery and the eval grader id `provenance_present` are deliberately unchanged, and graders accept either the new or the legacy label so pre-rename baselines stay gradable.
- **Render gate extended to the single-stock report skills** — `parallax-should-i-buy` and `parallax-score-explainer` now pass their drafted report through the shared deterministic render gate (`_parallax/render_gate.py`) as their mandatory last step, matching the portfolio family. Conventions §10 and both SKILL.md files updated.
- **i18n lang/register hand-offs** added to the client-facing report skills, and both translate skills gain a retail register alongside the existing institutional one.
- **White-label visual subset grows from six to seven keys** — `load_visual_branding()` gains a `render` key carrying deployment render defaults that feed the §13 audience mode; the full loader shape moves from 13 to 14 keys. `README.md`, `white-label/integration-pattern.md` (§1/§3/§9), and `schema.yaml` synced; the "About This Report" footer also gains an unconditional currency-basis line stating that figures are as reported with no base-currency conversion applied.
- **Docs vocabulary-consistency + audit-doc hygiene pass** — genericized a historical audit doc's evidence prose, and `CLAUDE.md` gains a rule that `docs/security/` audit docs must describe canary checks generically and never reproduce the literal canary terms, even inside a "found none" evidence line.

## 2026-06-14

### Added
- **`parallax-conventions.md` §0.0 Pre-flight** — resolve canonical `_parallax/`/house-view paths and confirm Parallax MCP schemas have registered **before the first data call**; abort with an explicit message on any gap. Converts the two recurring should-i-buy failure modes (missing convention files, MCP/schema race) from mid-run recoveries into one up-front check.
...
- **Stale `loader.md` §2.6 cross-references retargeted to §2** in `house-view/README.md`, `parallax-load-house-view/SKILL.md`, and `parallax-conventions.md` §0.3 — §2.6 never existed (numbering jumps §2 → §3).
...
- **Health-flag thresholds canonicalized** — `parallax-client-review` and `parallax-rebalance` evaluate all 5 flags at **holding level**, with `parallax-client-review/references/recommendation-matrix.md` as the binding condition source (its Concentration condition gains the top-3 >45% leg); `parallax-portfolio-checkup/references/health-flags.md` remains the canonical taxonomy and threshold-value source, and its portfolio-level weighted-average definitions remain the canon used by `parallax-portfolio-checkup` (and cited by `parallax-morning-brief`). `parallax-rebalance`'s Ground-truth Integrity block gains the mismatch-only render qualifier, matching its siblings. The audit's proposed `_parallax/portfolio-scoring-core.md` extraction was dropped — loader.md §3b + §5 rule 3 + health-flags.md already cover the shared content (see DECISIONS.md).
- **§9.1 standard disclaimer rendered by reference** — 25 SKILL.md files replace the inlined standard-disclaimer blockquote with `Render the standard disclaimer verbatim from parallax-conventions.md §9.1.` (view-aware skills fold the reference into their existing `Otherwise:` branch). Conventions §9.1 gains a "Consumer skill reference pattern" paragraph mirroring §9.2's; two new drift tests enforce no re-inlining and exemption-set integrity. Bespoke disclaimers (`parallax-pair-finder`, `parallax-scenario-analysis`, `parallax-concierge`) stay inline by design.
- **`loader.md` §5 rule 3 blesses the compact ground-truth display** — the name cross-check remains unconditional for every scored holding; the integrity table now renders only when a mismatch is detected (covering all scored holdings when it does). Matches existing consumer practice in `parallax-client-review`, `parallax-morning-brief`, and `parallax-scenario-analysis`.
- **`loader.md` §6.2 defines the `generate` audit-row fields** — `view_hash`, `provenance_hash`, and `parent_version_id` are now spec'd for `action == "generate"`, matching what `parallax-make-house-view` already emits. Spec-only change; the generate/save row-protocol question is deferred to a code PR.

### Fixed
- **Concierge router rows for post-launch skills** — the internal branch tables (concierge's stated routing logic) had no rows for `/parallax-credit-lens`, `/parallax-pair-finder`, `/parallax-cio-letter-prep`, or the house-view family, so those asks fell through to guesswork. Added: credit-lens + pair-finder rows in the Stock branch, cio-letter-prep in the Portfolio branch, and a house-view operations table (load / judge / make / stress / diff / compare). The user-facing menu stays at 4 branches.
- **`parallax-country-deep-dive` integrity wiring** — loader.md is now loaded unconditionally (§5 rule 3 ground-truth check, rule 6 AI disclosure, and §6 audit apply view or no view; the old gotcha only loaded it when a view was active, so the no-view path never reached rule 3). Batch B adds `get_company_info` per top-5 symbol as the ground-truth oracle with rule-3 mismatch recovery; the §6 audit row appends on every run (`applied_reason: "no_view"` when none); `build_stock_universe`/`macro_analyst` parameters are now named per conventions §0.2.
- **`parallax-macro-outlook` equities-mode ground-truth wiring** — the optional Batch C census renders per-holding scores and now carries the same rule-3 oracle calls and conditional Ground-truth Integrity block as country-deep-dive.
- **token-costs rows for the Set 5 pair** — `parallax-country-deep-dive` 34 → **29** (the old figure assumed 3 macro calls; the skill mandates one summary call; +5 company-info checks); `parallax-macro-outlook` 41 → **46** in equities mode (+5 company-info checks; 28 without equities unchanged).
- **Dead `_NINE_TWO_EXEMPT_SKILLS` entry** — the exemption set held `white-label-onboard`, which never matched the directory name `parallax-white-label-onboard`, so the §9.2 test gate failed on the exempted skill. Entries are now full directory names, and a new test asserts every entry names a real skill directory. `parallax-make-house-view` is added to the exemption with rationale in conventions §9.2 (config artifacts + Step 7 operator gate); `parallax-judge-house-view` is explicitly NOT exempt and its `report.md` now renders §9.2.
- **`parallax-halal-screen` phantom build mode** — the `"build halal portfolio in Southeast Asian equities"` Usage example advertised a workflow branch that doesn't exist. Removed; Usage now routes construction to `/parallax-portfolio-builder` with screening here afterward.
- **Doc references synced to the §5 rule 3 rename and skill-directory rename** — `parallax-morning-brief`, `parallax-rebalance`, and `parallax-thematic-screen` SKILL.md references to the loader.md §5 rule 3 "ground-truth panel" now say "ground-truth check" (matching the reworded rule); residual pre-rename skill names in `_parallax/house-view/` docs (`loader.md`, `README.md`, `auto-on-load-judge-pattern.md`) updated to full `parallax-*` directory names, including the stale `skills/load-house-view/SKILL.md` path in loader.md §"Framework components".
- **`greenblatt.md` universe default** — the profile spec described the default as mid-to-large-cap US equities; the dispatcher deliberately defaults to a sector-scoped query (`"US large-cap consumer staples"`) because broad universe queries time out. The spec now documents the production default, the timeout rationale, and the sector-by-sector merge path for broad screens. No dispatcher behavior change.
...
- **`analyze_portfolio` contract schema extended** — `contract_schemas.py` now covers ten additional response fields: `company_contribution`, `portfolio_summary`, `drawdown_analysis`, `performance_metrics`, `latest_holdings`, `sector_allocation`, `sector_contribution`, `time_period_returns`, `portfolio_scores`, `rolling_metrics`. The `concentration` key was renamed to `concentration_metrics` to match the fields-selector name. Mock updated to match.
