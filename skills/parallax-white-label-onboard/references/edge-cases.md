...
**URL rejected as non-public (`extract_from_url` returns `error: "URL destination is not public"`):** The destination policy rejected the URL itself, not the network — an intranet host, a private/loopback/link-local address, or a non-HTTP(S) scheme, applied to redirects and linked stylesheets too. Retrying or checking reachability will not help. Tell the user the source must be a publicly reachable HTTP(S) page, and offer PDF/PPTX/DOCX export of the brand guide or wizard mode instead. Do NOT fetch the URL through defuddle, WebFetch, or any other path to work around the rejection.
