...
| `view_hash` | `action` is one of `save`, `extend`, `re-pair`, `generate` | string (sha256 hex) | Canonical hash of the view body per schema.yaml §"view_hash computation". Matches `metadata.view_hash` written into the saved `view.yaml`, correlating the row to the saved bundle. `view_commit` enforces this whenever the row's write set includes `view.yaml` (covering `save`/`extend`/`re-pair`); for `generate`, which `parallax-make-house-view` emits directly without going through `view_commit`, this is a producer-side contract only. |
