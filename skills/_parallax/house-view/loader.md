...
| `view_hash` | `action` is one of `save`, `extend`, `re-pair`, `generate` | string (sha256 hex) | Canonical hash of the view body per schema.yaml §"view_hash computation". Matches `metadata.view_hash` written into the saved `view.yaml`, correlating the row to the saved bundle. |
