...
|---|---|---|
| `audit_chain.py` | `append_entry(audit_path, entry_data)`, `view_transaction(view_dir)` | Append-only hash-chained audit log. Each entry's `prev_entry_hash` links to the previous entry. RFC 8785 JCS canonicalization. Tampering with any entry breaks the chain on next verification. `view_transaction` is a `flock`-based context manager serializing a view read + audit append across threads and processes (used by stress and judge race guards). |
