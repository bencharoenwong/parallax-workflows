...
| `chain_prune.py` | `python3 chain_prune.py --before YYYY-MM-DD [--confirm]` | Retention CLI. Dry-run by default, `--confirm` to delete. Refuses symlinked chain_dir; declines files modified after process start; parses `--before` strictly. |
...
| `gate_present.py` | `GateContext`, `GateResult`, `run_gate_loop(...)` | Extracted Step 5 confirmation gate, shared by `parallax-load-house-view` and `parallax-make-house-view`. Drives the operator disposition loop (accept / edit / reject) and renders the canonical pre-save preview. Single source of truth for gate display + disposition handling across both writer skills. |
