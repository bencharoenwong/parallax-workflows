...
def mock_view(temp_house_view_dir: Path, sample_view_data: dict):
    ...
    return stress.View(
        data=sample_view_data,
        view_hash=fixture_hash,
        view_path=view_path,
...
def test_load_active_view_broken_chain(
    mock_view: stress.View, temp_house_view_dir: Path
):
...
def test_rule_triggers_rates_momentum(mock_view: stress.View, sample_rules_path: Path):
    ...
    triggered_result = next(
        r for r in results if r.rule_id == "rates_cutting_momentum_pos"
    )
    assert triggered_result.triggered
    assert not triggered_result.downgraded_from_stale_config


def test_rule_downgrades_on_stale_config(
    mock_view: stress.View, sample_rules_path: Path
):
...
def test_rule_growth_slowing_cyclicals_triggers(mock_view, sample_rules_path):
    ...
    assert any(
        r.triggered
        for r in results
        if r.rule_id == "growth_slowing_cyclicals_overweight"
    )
...
def test_rule_excludes_energy_em_commodity_triggers(mock_view, sample_rules_path):
    ...
    assert any(
        r.triggered
        for r in results
        if r.rule_id == "excludes_energy_em_commodity_overweight"
    )
...
@pytest.mark.parametrize(
    "cio_tilt, parallax_view, age_delta, expected_state",
    [
...
def test_resolve_cell_state_all_cases(
    cio_tilt, parallax_view, age_delta, expected_state
):
    """(ID: external-005 - 010) ★★★ All 6 cell state resolutions"""
    assert (
        stress.resolve_cell_state(cio_tilt, parallax_view, age_delta) == expected_state
    )
...
def test_render_artifact_full_report(mock_view):
    ...
    internal_results = [
        stress.RuleResult(rule_id="test_rule", triggered=True, evidence=["evidence"])
    ]
...
def test_rule_class_propagates_to_result(mock_view, sample_rules_path):
    ...
    results = stress.evaluate_internal_rules(mock_view, sample_rules_path)
    triggered = [
        r for r in results if r.triggered and r.rule_id == "rates_cutting_momentum_pos"
    ]
    assert triggered, "rates_cutting_momentum_pos rule should have triggered"
    assert (
        triggered[0].rule_class == "hard_stop"
...
def test_render_hard_stop_status(mock_view):
    ...
    results = [
        stress.RuleResult(
            rule_id="any_id", triggered=True, rule_class="hard_stop", evidence=["x"]
        ),
    ]
    report = render.render_artifact(
        mock_view.data["metadata"], results, {}, [], "hash123"
    )
    assert "🛑 HARD STOP" in report
    assert (
        "⚠️ TASTE" not in report.split("🛑 HARD STOP")[0]
    )  # not also rendered as TASTE
...
def test_audit_ts_format_has_z_suffix(mock_view):
    ...
    with _mock.patch.object(
        stress.audit_chain, "append_entry", side_effect=fake_append
    ):
...
def test_build_recommended_deltas_shape():
    ...
    resolutions = [
        {
            "market": "japan",
            "dim": "sectors.utilities",
            "cio_tilt": 1,
            "parallax_view": -1,
            "state": "DIVERGENT_STALE",
            "parallax_summary": "Parallax: Japan utilities rank 4",
        },
        {
            "market": "us",
            "dim": "factors.momentum",
            "cio_tilt": 1,
            "parallax_view": 1,
            "state": "ALIGNED",
            "parallax_summary": "Parallax: momentum dominant",
        },
        {
            "market": "india",
            "dim": "factors.profitability",
            "cio_tilt": 2,
            "parallax_view": -1,
            "state": "DIVERGENT_STALE",
            "parallax_summary": "Parallax: quality 0.50",
        },
    ]
    deltas = stress.build_recommended_deltas(
        resolutions, cio_age_days=56, parallax_age_days=1
    )
    assert (
        len(deltas) == 2
    ), f"expected 2 deltas (only DIVERGENT_STALE); got {len(deltas)}"
    for d in deltas:
...
def test_audit_entry_omits_deltas_when_none(mock_view):
    ...
    stress.append_stress_audit(
        mock_view, summary={"internal_pass": True}, applied=False
    )
    entries = audit_chain.verify_chain(mock_view.audit_path)
    stress_entries = [e for e in entries if e.get("action") == "stress_test"]
...
def test_classify_mcp_unknown_error_with_success_true_is_silent():
    ...
    for err in ("permission_denied", "tool_not_found", "internal_error", "forbidden"):
        ...
        assert (
            state == "PARALLAX_SILENT"
        ), f"{err} should be PARALLAX_SILENT, got {state}"
...
def test_validate_deltas_styles_four_segment_valid():
    ...
    deltas = [
        {
            "kind": "informational",
            "path": "tilts.styles.cap_size.large",
            "market": None,
            "cio_value": 1,
            "parallax_signal": -1,
            "parallax_summary": "",
...
def test_validate_deltas_string_parallax_signal_rejected():
    ...
    deltas = [
        {
            "kind": "informational",
            "path": "tilts.macro_regime.growth",
            "market": None,
            "cio_value": "slowing",
            "parallax_signal": "slowing",
            "parallax_summary": "",
            "stress_state": "DIVERGENT_STALE",
            "cio_age_days": 1,
            "parallax_age_days": 1,
...
def test_validation_failure_disposition_roundtrips_through_audit(mock_view):
    ...
    bad_deltas = [
        {
            "kind": "informational",
            "path": "factors.momentum",  # no tilts. prefix
            "market": None,
            "cio_value": 1,
            "parallax_signal": -1,
            "parallax_summary": "",
...
def test_audit_validation_errors_roundtrip(mock_view):
    ...
    errors = [
        {"index": 0, "field": "path", "reason": "path must start with 'tilts.'"},
        {
            "index": 1,
            "field": "kind",
            "reason": "kind must be 'informational' or 'global'",
        },
    ]
...
def test_validate_deltas_styles_4seg_still_runs_signal_check():
    ...
    assert ok is False
    assert any(
        e["field"] == "parallax_signal" for e in errors
