...
| `ask-operator` | Render the question and its choices in prose and stop. Within that turn the gate is NOT passed and nothing is persisted. An explicit operator answer in a later turn satisfies the gate; record in the audit row which form the answer took. |
