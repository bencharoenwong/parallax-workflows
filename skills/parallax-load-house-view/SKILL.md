...
- PDF input — `load-reference` the file directly, in page ranges for >10 pages (Step 2); do NOT push a PDF through a text extractor (we want figure context)
- PDF extraction is not deterministic, especially on large chunked PDFs — see the determinism note in Step 2.
- URL input — `fetch-url` (binding per conventions §14.2)
...
- Ask uploader_role and basis_statement via `ask-operator` — these are required-at-institutional fields and need explicit human input
- Auto-applied macro_regime → factor tilts (loader.md §3) MUST be surfaced to the uploader at the gate, not silently applied
- Confirmation gate persists a pre-edit snapshot (Step 5a) when uploader chooses 'Edit specific fields' — writes extractor's pristine draft to `.archive/<version>/pre_edit.yaml` alongside the superseded view. No-edit confirmations skip this.
- Every extraction attempt (Step 5b) logs an `extraction_attempt` audit entry to `audit.jsonl` — whether or not it becomes a save. Capture disposition (confirmed/edited/re_extracted/rejected) + draft_yaml_hash per loader.md §6.2. Append it with `audit_chain.append_entry`, never with `write-artifact` (Step 5b names the call).
- Persist (Step 6 item 9, Stage 1) computes a `version_diff` block vs `parent_version_id` and stashes it on the save audit entry. Only when this save supersedes a prior version.
- Calibration manifest: Invoke `manifest_cache.load_manifest()` during Step 6 (Persist) to resolve active calibration. Handle `DeadStateNoFallback` or signature errors by logging a warning and falling back to the bundled-values default.
...
- **Persistence fails closed.** Nothing is written unless `ask-operator` returned an explicit confirmation in this session (conventions §14.3). Without that primitive, render the gate in prose and stop; a later explicit yes satisfies the gate.
- Every host interaction is a host primitive from `parallax-conventions.md` §14 (bindings §14.2, fail-open §14.3). This skill makes no Parallax calls.
...
2. `load-reference` `_parallax/house-view/schema.yaml` (canonical structure) and `_parallax/house-view/loader.md` (consumer expectations) before extraction.
3. Bind the primitives this skill uses: `load-reference`, `fetch-url`, `ask-operator`, `write-artifact`, `run-shell`, `read-config`. If `ask-operator` or `write-artifact` is absent, this skill can extract and display a draft but cannot save; say so before Step 1.
   <!-- host-note -->
   Claude Code: `load-reference` = `Read` (use `pages` for PDFs); `fetch-url` = `defuddle` when Bash is available, else `WebFetch`; `ask-operator` = `AskUserQuestion`; `write-artifact` = `Write`; `run-shell` = `Bash`.
   <!-- /host-note -->
4. `read-config`: `$PARALLAX_HOUSE_VIEW_DIR`, else `~/.parallax/active-house-view/` — the same resolution `view_commit` and `view_status` use.

### Step 1 — Resolve inputs

Parse the argument: a file path, a URL, an operational flag (`--status`, `--clear`, `--extend`, `--re-pair`, `--edit`, `--export`, `--why`, `--version-history` → `## Modes`, no extraction), or nothing (wizard mode). A path that does not exist or a URL that does not fetch stops here with the reason; never guess content.

### Step 2 — Fetch (parallel batches)

No Parallax batches: the fetch is the source document itself. If a path or URL was given, read the source via the matching primitive. For PDFs:
- **≤10 pages:** `load-reference` the file in one page-ranged call; proceed to Step 3.
- **>10 pages:** use streaming extraction — read in 5-page chunks (`load-reference` with page range `N-(N+4)`), parse YAML incrementally, merge results. Track `extraction_confidence` per chunk (if a chunk fails to parse, flag ≤ 0.5 for that chunk and continue). Merged result proceeds to Step 3 with average confidence score across chunks.

**Determinism note.** Chunked extraction on a large PDF is the least deterministic path in this skill — re-running the same document can produce different draft values. Do not present it to the uploader as a precise read of the source. Surface a lower confidence when the source is long or dense, and rely on the Step 5 confirmation gate, not on repeated extraction attempts, to reach a value the uploader trusts.

For URLs: `fetch-url`. Skip the wizard and proceed to Step 3.

If no source was given (wizard mode): walk the uploader through the schema interactively in the order below, using one `ask-operator` call per numbered group (not eight separate prompts). Skip any dimension the uploader leaves neutral.
...
- The source is readable end to end: every PDF chunk parsed or was flagged ≤ 0.5; a URL fetch returned body text, not an error page. A source that yields no text stops here — ask for pasted text.
- `schema.yaml` loaded and its `view_hash` computation section is present; loader.md §3 multiplier mapping is present (the gate shows its auto-applied deltas).
- Wizard mode: every numbered group was asked or explicitly skipped.

### Step 4 — Compute (extract structured tilts)
...
**Required uploader-supplied fields** (cannot be extracted from source — ask via `ask-operator` after extraction):
...
> **Shared module.** The gate display + disposition loop lives in `_parallax/house-view/gate_present.py` and `/parallax-make-house-view` uses it too. This Step describes how `parallax-load-house-view` *uses* the module; the module itself is the source of truth for display rendering and disposition vocabulary. Step 5a (pre-edit snapshot persistence) and Step 5b (extraction_attempt audit logging) remain caller-side responsibilities — the module returns the snapshot in `GateResult` but never writes audit rows or `.archive/<...>/pre_edit.yaml`. Step 5b's row is appended with `audit_chain.append_entry` (see §5b); it does not go through `view_commit`, which derives `action` from its `--mode` and has no extraction-attempt mode.

JIT-load `_parallax/house-view/gate_present.py` and construct a `GateContext`:

- `source_label`: the filename / URL / `"wizard"` identifier from Steps 1–2.
...
- `extraction_attempt_action=True` (this skill always logs the `extraction_attempt` row per §5b below).
- `disposition_options=["confirm", "edit", "re_extract", "reject"]`.

Call `gate_present.run_gate_loop(draft, context, dispose_fn=..., edit_fn=...)`. The two callbacks bridge to `ask-operator`:

- `dispose_fn(prompt) -> str` — print `prompt.display` verbatim, then ask `prompt.question` with `prompt.options` via `ask-operator`. Return the chosen disposition keyword.
- `edit_fn(current_draft, context) -> (edited_draft, edit_notes | None)` — loop on `ask-operator` per flagged field (the LOW-CONFIDENCE block from the rendered prompt is the suggested order). After all edits land, optionally ask "One line on what you changed and why? (optional)" and pass the response as `edit_notes`. Return the post-edit draft. The module re-renders the gate and re-invokes `dispose_fn` until the uploader confirms or branches to a terminal disposition.

The module returns a `GateResult` with `disposition` set to one of `"confirm"`, `"edited"`, `"re_extracted"`, or `"rejected"`. Branch as follows:

- **`"confirm"`** — proceed to Step 6 with `result.final_draft`. No pre-edit snapshot to persist. Write the `extraction_attempt` audit row per §5b with `disposition="confirmed"` and `draft_yaml_hash` = sha256(canonical) of `result.final_draft`.
- **`"edited"`** — proceed to Step 5a using `result.pre_edit_snapshot` (the pristine pre-edit draft, which the module captured automatically on first entry into the edit branch) and optionally `result.edit_notes`. Then write the `extraction_attempt` audit row per §5b with `disposition="edited"` and `draft_yaml_hash` = sha256(canonical) of the **pre-edit** snapshot — not the post-edit confirmed draft (that hash goes into `view.yaml` via Step 6's `view_hash`). The pair `extraction_attempt.draft_yaml_hash` + `save.view_hash` is the audit signature of "what the extractor produced → what the uploader shipped." Proceed to Step 6 with `result.final_draft`.
- **`"re_extracted"`** — discard any holding buffer; do not write `pre_edit.yaml`. Write the `extraction_attempt` audit row per §5b with `disposition="re_extracted"`, the rejected draft's hash, and the hint (collected via a follow-up `ask-operator`). Re-run Step 4 with the hint added to extraction context and return to Step 5.
- **`"rejected"`** — discard any holding buffer. Write the `extraction_attempt` audit row per §5b with `disposition="rejected"`. Abandon; do not write `view.yaml`.

#### Step 5a — Pre-edit snapshot (Layer 2)
...
2. After all edits land AND the uploader `Confirm`s the edited version, write the pre-edit snapshot to `~/.parallax/active-house-view/.archive/<new_view_id>-<new_version_id>/pre_edit.yaml`. (The directory is the SAME one Step 6 item 9 creates to archive the superseded view — both files share the slot.)
...
If the uploader confirms the pristine draft (no edits), skip Step 5a entirely — there's nothing to snapshot.

#### Step 5b — Extraction attempt logging (Layer 5, folded into audit.jsonl)
...
**How to write it.** Append the row with `audit_chain.append_entry` — the append-only path that takes the view's `flock`, computes `prev_entry_hash`, and never replaces the file. It is the one sanctioned direct write to `audit.jsonl`; `write-artifact` is not (see Step 6 Stage 2). `view_commit` cannot produce this row: it derives `audit_entry["action"]` from `--mode`, and there is no `extraction_attempt` mode. Run exactly:
...
9. **Write (atomic-safe per-file pattern)**:
   ...
   - **Stage 2 — Commit.** Use the staging directory `$HOME/.parallax/.staging` — create it with `mkdir -p "$staging" && chmod 700 "$staging"` (a bare `mkdir` inherits the shell's umask, typically `0755`; the `chmod 700` is what protects the source-extracted content in `provenance.yaml` while it sits outside the view directory, and a fixed path under `$HOME` avoids the world-writable-directory exposure `/tmp` carries). Write the three artifacts into `$staging` with `write-artifact` (`view.yaml` with frontmatter fields set per the earlier substeps, `prose.md` with frontmatter **four fields in this order** — `paired_yaml_hash`, `prose_body_hash`, `view_id`, `version_id` — frontmatter being the only part of the file NOT covered by `prose_body_hash`, and `provenance.yaml`), **and write the plan JSON itself into `$staging/plan.json` with `write-artifact` too** — never type the plan into a shell heredoc. The plan carries document-derived text (the `version_diff` values come from the CIO draft's `tilts`/`excludes`), and a heredoc would hand that text to the shell for command substitution; a file written by `write-artifact` never passes through a shell. Because the staging path is known, every `path` in the plan is a literal absolute path (no `~`, no variables — `view_commit` reads the file as-is). Never write into `~/.parallax/active-house-view/` yourself. Remove `$staging` unconditionally after the commit (shown in the block) — the contents are reconstructible, so leaving source-extracted output behind on a failed commit is the worse trade — and end the block with `exit "$status"` so `run-shell` reports `view_commit`'s own exit code rather than the cleanup's.

     **`audit.jsonl` is append vs replace, not touch vs don't-touch.** Never stage, rename, copy over, or overwrite it, and never write it with `write-artifact` — every one of those replaces the file, which swaps the inode out from under the append path and truncates the chain into something that still verifies green. Appending through `audit_chain.append_entry` is the one sanctioned direct write (that is how Step 5b's `extraction_attempt` row lands, and `view_commit` itself appends through the same call). Every commit of the canonical artifacts — `view.yaml`, `prose.md`, `provenance.yaml` — goes through `view_commit`, which appends its own witnessing row. Then run exactly:

     ```bash
     staging="$HOME/.parallax/.staging"
     mkdir -p "$staging" && chmod 700 "$staging"
     # write view.yaml, prose.md, provenance.yaml, and plan.json into "$staging" via write-artifact, then:
     ...
     `plan.json` (written with `write-artifact`; `<home>` is the literal expansion of `$HOME`):
...
10. Not a separate item — `version_diff` computation happens in item 9's Stage 1, before archiving.
11a. **Emit Reasoning Chain (deferred, non-blocking):**
    - This step can be deferred to async completion AFTER the save confirmation is returned to the user. It does not block the post-save summary (Output Format).
...
| `--clear` | **Archives** the current view, then removes **all three** artifacts (`view.yaml`, `prose.md`, `provenance.yaml`) via `view_commit --mode clear`. All three are removed — leaving `provenance.yaml` behind would let a later export silently bundle a prior view's stale evidence, since `audit_export.py` only raises when the file is *missing*. Writes an audit row (`destination` + `reason` required); `destination` is the archive directory, so the archive must happen first. See invocation below. |
| `--extend <date>` | Update `metadata.valid_through` only. Bump `version_id`. Recompute `view_hash` (should be unchanged since tilts/excludes unmodified) and re-write `prose.md` frontmatter. Commits both files plus an audit row via `view_commit --mode extend`. See invocation below. |
| `--re-pair` | Recompute `view_hash` from current `view.yaml` and `prose_body_hash` from current `prose.md` body. Commits the updated `prose.md` frontmatter (`paired_yaml_hash` AND `prose_body_hash`) plus an audit row via `view_commit --mode re-pair`. Use this after a manual prose edit (body or YAML) when the edit was intentional; the command re-anchors both hashes in one step. Note that re-pair intentionally blesses whatever is currently on disk — run only after you have reviewed the edit. See invocation below. |
| `--edit` | Open `view.yaml` in `$EDITOR` (default: `vi`). On save, re-run Steps 5–6 (confirmation gate + persist) using the edited content as the draft. |
...
`--re-pair` (writes `prose.md` only — `view.yaml` is untouched). Create the staging directory, write the file and `plan.json` into it with `write-artifact`, then commit:

```bash
staging="$HOME/.parallax/.staging"
mkdir -p "$staging" && chmod 700 "$staging"
# write prose.md and plan.json into "$staging" via write-artifact, then:
...
2. Resolve the view directory exactly as the command below does — `$PARALLAX_HOUSE_VIEW_DIR` if set, else `~/.parallax/active-house-view/` — and copy `view.yaml`, `prose.md`, and `provenance.yaml` (each one that exists) into `<view_dir>/.archive/<view_id>-<version_id>/`, creating the directory `0700`. Do not hardcode the default path: the command omits `--dir` and clears whichever directory that variable resolves to, so archiving out of a different one would again make `destination` name a location that does not hold what was deleted. Use `load-reference` + `write-artifact`, or `cp`; this is a copy, not a move — `view_commit` does the removal.
...
The post-save summary below after Step 6; §"Status block" above for `--status`; for other modes, the requested success/failure message.

### Post-save summary

Output to user after a successful Step 6:
...
**§9.2 exemption:** This skill is listed in `_NINE_TWO_EXEMPT_SKILLS` (`_parallax/white-label/tests/test_integration_pattern_referenced.py`) and does not render the §9.2 banner itself — it emits a `view.yaml` configuration artifact, and any LLM-extracted content is gated by Step 5's operator confirmation before it can flow downstream. Each downstream consumer skill renders §9.2 in its own output when it loads the saved view (see `parallax-conventions.md` §9.2 exemption rationale).

## Failure modes

- `ask-operator` absent, or no explicit confirmation this session: render the gate in prose and stop; nothing is written (fails closed, conventions §14.3). Never treat silence or a summary request as confirmation.
- `write-artifact` or `run-shell` absent: extraction and the gate still run; the save cannot; say so at Step 0 and stop before Step 6.
- `view_commit` exit 2: refused, nothing written; surface the message verbatim (a moved `expected_identity` means the active view changed since Stage 1). Exit 1: follow the recovery named in the message; never hand-edit `audit.jsonl`.
- `manifest_cache.load_manifest()` raises: log the warning, `status="PHASE_0_FALLBACK"`, `calibration_status="heuristic_phase0"`.
- Archive copy fails in `--clear`: stop before the clear; an unrecoverable clear is worse than none.
- Chunked PDF disagreement across runs: lower confidence, let the gate settle the value.

## Done when

- The operator saw the rendered gate and answered through `ask-operator`; the `extraction_attempt` row was appended with the matching disposition.
- On confirm: `view_commit --mode save` exited 0 and printed the audit row; `view.yaml`, `prose.md`, `provenance.yaml` exist `0600` under the resolved view directory; the staging directory is gone; the reasoning chain was emitted.
- The post-save summary (Output Format) was rendered, or the mode's success/failure message for a flag run.
