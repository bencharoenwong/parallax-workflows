...
Read `skills/_parallax/skill-structure-conventions.md` first — it defines the step spine, the host-portability rule (name host primitives from `parallax-conventions.md` §14, never host tools), the authority header for shared files, and the two closing sections every orchestrator carries. Then mirror the structure of one of the newer skills:
