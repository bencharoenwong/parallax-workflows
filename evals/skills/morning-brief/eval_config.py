...
_MACRO_TERMS = re.compile(
    ...
    r"cpi|ppi|fomc|outlook|tilt|overweight|underweight|growth|recession)\b",
    re.I,
)
...
def _c_macro_snapshot_grounded(t, spec) -> Check:
    ...
    if not t.called("macro_analyst"):
        return Check(
            "macro_snapshot_grounded", True, "macro_analyst not called (vacuous)"
        )
    ...
    return Check(
        "macro_snapshot_grounded",
        ok,
        f"snapshot_present={bool(snap)} body_terms={bool(_MACRO_TERMS.search(body))}",
    )
...
def _c_word_limit(t, spec) -> Check:
    ...
    marks = [
        ...
        for m in re.finditer(
            r"(?i)AI-assisted output|AI-interaction disclosure|not investment advice",
            prose,
        )
    ]
