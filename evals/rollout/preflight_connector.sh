...
# Parallax connector, by asking it to run one trivial health call. Exits 0 only when both
...
claude -p "Call the Parallax check_api_health tool exactly once and report only its status field. Do nothing else." \
...
# 3) The health tool actually ran (connector reachable, not just declared)
if ! grep -qi 'check_api_health' "$TMP"; then
  echo "PREFLIGHT FAIL: Parallax connector declared but check_api_health never executed (tool blocked or unreachable)." >&2
  echo "  fix: ensure ALLOWED_TOOLS permits the Parallax tools so a non-interactive run can call them." >&2
  exit 4
fi

echo "PREFLIGHT OK: Parallax connector loaded and authenticated; health tool responded." >&2
exit 0
