...
| `policy_fallback_tier` | `client_policy_applied` is present | string (enum) | One of `weights_only`, `full`, `partial_dimensions`, `invalid`. `invalid` is required specifically when `client_policy_applied == false` (a policy was supplied but was unusable — unreadable, unparseable, or resolved to `adaptation.py`'s internal `no_policy` tier). Absence of the field means no policy was supplied at all. |
