...
6. **AI-interaction disclosure** (bottom, immediately above the view-aware disclaimer — REQUIRED whether or not a view is active; applies to every consumer skill output): Render the canonical banner from `_parallax/parallax-conventions.md §9.2`. The banner is rendered by reference, not inlined — a single source of truth in §9.2 lets counsel sign-off and the forthcoming attribution-API endpoint propagate to all consumer skills with one edit.
   ...
   The current §9.2 banner accurately distinguishes deterministic quantitative pipelines from LLM-generated qualitative content (including the LLM-backed Parallax MCP services — news synthesis, macro commentary, assessment — as well as the orchestrating model's narrative and recommendations), and points readers at the About This Report footer for this output's branding and data-basis state. It supersedes the earlier "AI generated factor scores" placeholder, which was technically wrong — factor scores come from CG's deterministic pipeline, not from an LLM.
...
| `action` | string (enum) | One of: `save`, `clear`, `extend`, `re-pair`, `edit`, `consume`, `extraction_attempt`, `stress_test`, `generate`, `judge`, `attribution`. `attribution` is emitted by `parallax-house-view-attribution` once per run and is always applied=false (attribution is read-only ex-post measurement; it never modifies the view). `consume` is the default for any consumer-skill invocation (portfolio-builder, should-i-buy, etc.). `extraction_attempt` logs ingestion drafts whether or not they became saves — captures retries, edits, and rejects. `stress_test` is emitted by `parallax-stress-house-view` once per run (applied=false unless the CIO opts to update the view at the Phase 4 gate, in which case a separate `save` entry from `parallax-load-house-view` follows). `generate` is emitted by `parallax-make-house-view` once per successful synthesis (applied=true on the corresponding save row; the `generate` row itself is logged via the §6.2 fields below and a separate `save` row from `parallax-load-house-view` lands the view). `judge` is emitted by `parallax-judge-house-view` once per run and is always applied=false (judges never modify the view). The others are operational events from `parallax-load-house-view`. Consumers that encounter an unrecognized `action` value MUST skip the row (forward-compatibility). |
...
| `applied_reason` | `applied == false` AND (validation passed OR no view present) | string | Why tilts weren't applied. Typical values: `"single-stock consumer (loader.md §7.1/§7.2/§7.3)"`, `"divergence refusal (loader.md §5 rule 4)"`, `"operational action (no consume)"`, `"no_view"` (consume event with no active view — `view_id`/`version_id` null per §6.1). |
...
| `view_hash` | `action` is one of `save`, `extend`, `re-pair`, `generate` | string (sha256 hex) | Canonical hash of the view body per schema.yaml §"view_hash computation". Matches `metadata.view_hash` written into the saved `view.yaml`, correlating the row to the saved bundle. `view_commit` enforces this whenever the row's write set includes `view.yaml`, which covers `save` and `extend`. It does not cover `re-pair`, whose write set is `prose.md` only. There `view_hash` is supplied as an `expected_identity` key — recomputed from the on-disk `view.yaml` and compared under the lock before anything is written — rather than being enforced as a row field. A `re-pair` row is still expected to carry `view_hash` per the required-when column above, and when it does, the prose frontmatter's `paired_yaml_hash` is checked against it; but that check is conditional on the field being present, so it is a producer-side contract for this mode too. For `generate`, which `parallax-make-house-view` emits directly without going through `view_commit`, this is a producer-side contract only. |
...
| `attributed_view_id` | `action == "attribution"` | string (uuid v4) | The `view_id` of the view family being attributed. |
| `attributed_version_id` | `action == "attribution"` | string (uuid v4) | The specific version attributed, or the latest version when the window spans several (per-segment detail lives in the report bundle). |
| `attribution_window` | `action == "attribution"` | object | `{start, end}` ISO dates — `[effective_date, min(valid_through, today)]` clipped to available data. |
| `attribution_summary` | `action == "attribution"` | object | `{segments: int, holdings_covered: int, holdings_dropped: int, chains_used: int, active_return_bps, model_active_bps, selection_residual_bps, per_tilt: [{group, realized_contribution_bps}], counterfactual_quality: "exact"\|"approximate", reference_portfolio_hash}`. `active_return_bps` is the realized tilted-vs-neutral difference from the chain's actual weights; `model_active_bps` is the multiplier-model-implied difference (the quantity the `per_tilt` Shapley contributions sum to exactly); `selection_residual_bps = active_return_bps − model_active_bps`. All fields except `chains_used` and `reference_portfolio_hash` come verbatim from `attribution.merge_segments` — do not compute additional return figures inline. The evidence payload for the `empirical_phase1` on-ramp — per-tilt realized contribution, not just a headline number. |
| `report_hash` | `action == "attribution"` | string (sha256 hex) | Hash of the rendered `report.md`. A signed calibration manifest may cite this value as its `backtest_ref`, which is how attribution evidence feeds the §5.1a `empirical_phase1` promotion — promotion itself remains a signed-manifest operator action, never automatic. |
| `client_policy_applied` | `action == "consume"` AND a client policy input was supplied | bool | True when policy figures rendered; false when a policy was supplied but resolved to tier `no_policy`. Absent when no policy was supplied at all. |
| `client_ref` | `client_policy_applied` is present | string (≤64 chars) | The policy's own pseudonymous client reference (`metadata.client_ref`), which is what §6.3 permits the client policy to contribute. Never a client name, an account number, or any other direct identifier. Correlates rows belonging to one mandate without identifying the client. |
| `policy_hash` | `client_policy_applied` is present AND the policy document parsed as a mapping | string (sha256 hex) | Canonical hash of the policy's `mandate` + `adaptation` subtrees (`_parallax/client-policy/adaptation.py`'s `compute_policy_hash`). Correlates the row to the policy artifact without logging its content. Omitted — not logged as an empty string — when the policy was unreadable, unparseable, or parsed but was not a mapping (`adaptation.py` reports `policy_hash = ""` in exactly those cases). |
| `policy_fallback_tier` | `client_policy_applied` is present | string (enum) | One of `weights_only`, `full`, `partial_dimensions`, `invalid`, `unavailable`. **Translation (total, no other value is legal):** the helper's four tiers map identity for `weights_only` / `full` / `partial_dimensions`, and its `no_policy` tier — a policy that was supplied but is unusable, whether unreadable, unparseable, or schema-invalid — maps to the audit value `invalid`; `no_policy` itself is never written to the row. `invalid` is required specifically when `client_policy_applied == false`. `unavailable` covers the remaining state: a policy that was supplied and valid so far as known, whose helper invocation failed at runtime, so no tier was ever resolved — `invalid` would falsely describe that policy as unusable. Absence of the field means no policy was supplied at all. |
| `resolved_k` / `k_source` | `client_policy_applied == true` | float / string | The sizing factor actually used and its origin (`preset:<name>`, `explicit`, `default`). Every rendered result discloses these; the audit row records them. |
...
- **No PII.** `query_summary` is the only user-derived content; truncate at 200 chars and do not log account IDs, client names, or full holdings arrays. Client policy contributes `client_ref` and `policy_hash` only — client names, account identifiers, and full policy bodies are never logged.
...
{"schema_version":1,"ts":"2026-04-24T06:04:00Z","view_id":null,"version_id":null,"skill":"parallax-country-deep-dive","action":"consume","applied":false,"applied_reason":"no_view","query_summary":"/parallax-country-deep-dive Japan","ground_truth_mismatches":0}
```
