# Step 5 — Confirmation Gate Display & Edit Procedure

The four-option `ask-operator` set, the mismatch-resolution rule, and the gate-is-REQUIRED decision point stay in SKILL.md Step 5. This file is the full display template and the per-option procedures.
...
3. For each named field, ask for the new value via `ask-operator`.
...
5. Re-run Step 3 validation on the edited draft.
...
- Return to Step 1 with the new source.
